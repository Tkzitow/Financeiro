...
//    result := 12;
  ...
  if (ano_busca = ano_atual) and (ano_busca <> ano_inicio) then
    result := formatdatetime('mm', now).ToInteger - 1
  else if (ano_busca <> ano_atual) and (ano_busca = ano_inicio) then
    result := 13 - formatdatetime('mm', data_inicio).ToInteger
  else if (ano_busca = ano_atual) and (ano_busca = ano_inicio) then
    result := (formatdatetime('mm', now).ToInteger - formatdatetime('mm', data_inicio).ToInteger)
  else
    result := 12;
...
begin
  try
    ...
    if quantidadeMesesparaMedia < 1 then
      qtd_mes_para_media := 1
    else
      ...
      begin
        ...
        if entrada_or_saida then
          begin
            ...
            sql_janeiro := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 01 ' + sql_ano + ') as janeiro,';
            sql_fevereiro := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 02 ' + sql_ano + ') as fevereiro,';
            sql_marco := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 03 ' + sql_ano + ') as marco,';
            sql_abril := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 04 ' + sql_ano + ') as abril,';
            sql_maio := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 05 ' + sql_ano + ') as maio,';
            sql_junho := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 06 ' + sql_ano + ') as junho,';
            sql_julho := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 07 ' + sql_ano + ') as julho,';
            sql_agosto := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 08 ' + sql_ano + ') as agosto,';
            sql_setembro := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 09 ' + sql_ano + ') as setembro,';
            sql_outubro := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 10 ' + sql_ano + ') as outubro,';
            sql_novembro := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 11 ' + sql_ano + ') as novembro,';
            sql_dezembro := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item and month(data_entrada) = 12 ' + sql_ano + ') as dezembro,';
            sql_total := '(select sum(valor_entrada) from entradas ' + sql_join + sql_usuario + ' and ' + item_str + ' = nome_item ' + sql_ano + ') as total';
          ...
          begin
            ...
            sql_item := item_str + ' as nome_item,';
            sql_janeiro := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 01 ' + sql_ano + ') as janeiro,';
            sql_fevereiro := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 02 ' + sql_ano + ') as fevereiro,';
            sql_marco := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 03 ' + sql_ano + ') as marco,';
            sql_abril := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 04 ' + sql_ano + ') as abril,';
            sql_maio := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 05 ' + sql_ano + ') as maio,';
            sql_junho := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 06 ' + sql_ano + ') as junho,';
            sql_julho := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 07 ' + sql_ano + ') as julho,';
            sql_agosto := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 08 ' + sql_ano + ') as agosto,';
            sql_setembro := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 09 ' + sql_ano + ') as setembro,';
            sql_outubro := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 10 ' + sql_ano + ') as outubro,';
            sql_novembro := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 11 ' + sql_ano + ') as novembro,';
            sql_dezembro := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item ' + sql_item_fixo +' and month(data_saida) = 12 ' + sql_ano + ') as dezembro,';
            sql_total := '(select sum(valor_saida) from saidas ' + sql_join + sql_usuario +  ' and ' + item_str + ' = nome_item '+ sql_item_fixo + sql_ano + ') as total';
