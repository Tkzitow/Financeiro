...
type
  Tfml_comparativo = class(TForm)
    ...
    cbAnoMes1Comparativo: TComboBox;
    cbAnoMes2Comparativo: TComboBox;
    Label6: TLabel;
    Label7: TLabel;
...
begin
  try
    ...
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoMes1Comparativo.Text);
...
begin
  try
    ...
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoMes2Comparativo.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
...
begin
  ...
  try
    for I := 0 to High(arrFinalidade) do
      begin
        query.Query1.SQL.Clear;
        query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal from SAIDAS where ID_FINALIDADE = :pFinalidade and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
        ...
        query.Query1.SQL.Clear;
        query.Query1.SQL.Add('select sum(VALOR_SAIDA) / sum(QUANTIDADE) as media from SAIDAS where ID_FINALIDADE = :pFinalidade and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
        query.Query1.ParamByName('pFinalidade').AsInteger := dados.localizar_id(arrFinalidade[i], 1);
        query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes1_comparativo.Text);
        query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoMes1Comparativo.Text);
        query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
...
begin
  try
    ...
    query.Query1.SQL.Clear;
    query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal, sum(QUANTIDADE) as qtd from SAIDAS inner join FINALIDADE on SAIDAS.ID_FINALIDADE = FINALIDADE.ID_FINALIDADE '+'where FINALIDADE.FINALIDADE like ''Investimentos'' and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and SAIDAS.ID_USUARIO = :pUser');
    query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes1_comparativo.Text);
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoMes1Comparativo.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
...
begin
  ...
  try
    for I := 0 to High(arrItem) do
      begin
        query.Query1.SQL.Clear;
        query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal from SAIDAS where ID_ITEM = :pItem and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
        ...
        query.Query1.SQL.Clear;
        query.Query1.SQL.Add('select sum(VALOR_SAIDA) / sum(QUANTIDADE) as media from SAIDAS where ID_ITEM = :pItem and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
        query.Query1.ParamByName('pItem').AsInteger := dados.localizar_id(arrItem[i], 2);
        query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes1_comparativo.Text);
        query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoMes1Comparativo.Text);
        query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
...
begin
  ...
  if (ano.ToInteger <> ano_inicio.ToInteger) and (ano_inicio.ToInteger > 2015) then
    begin
      ...
      for I := 0 to High(sequencia_ano) do begin
        cbAnoMes1Comparativo.Items.Add(inttostr(sequencia_ano[i]));
        cbAnoMes2Comparativo.Items.Add(inttostr(sequencia_ano[i]));
      end;
    end
    else
    begin
      for I := 0 to (ano.ToInteger - 2023) do begin
        cbAnoMes1Comparativo.Items.Add(ano);
        cbAnoMes2Comparativo.Items.Add(ano);
  ...
  cbAnoMes1Comparativo.Text := formatdatetime('yyyy', now);
  cbAnoMes2Comparativo.Text := formatdatetime('yyyy', now);
end;
...
begin
  ...
  try
    for I := 0 to High(arrOrigem) do
      begin
        query.Query1.SQL.Clear;
        query.Query1.SQL.Add('select sum(VALOR_ENTRADA) as valorTotal from ENTRADAS where ID_ORIGEM = :pOrigem and month(DATA_ENTRADA) = :pMes and year(DATA_ENTRADA) = :pAno and ID_USUARIO = :pUser');
