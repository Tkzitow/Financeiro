...
type
  TFmlRegistros = class(TForm)
    ...
    cbRegistroAnoSaida: TComboBox;
    cbRegistroAnoEntrada: TComboBox;
    ...
    procedure dbGridSaidaRegistroDblClick(Sender: TObject);
...
begin
  ...
  cbDataSaidaRegistro.Items.Add('Ano');
  ...
  cbDataEntradaRegistro.Items.Add('Ano');
  ...
  if (ano.ToInteger <> ano_inicio.ToInteger) and (ano_inicio.ToInteger > 2015) then begin
    ...
    for I := 0 to High(sequencia_ano) do begin
      cbRegistroAnoSaida.Items.Add(inttostr(sequencia_ano[i]));
      cbRegistroAnoEntrada.Items.Add(inttostr(sequencia_ano[i]))
    end;
  end
  else
  begin
    for I := 0 to (ano.ToInteger - 2023) do begin
      cbRegistroAnoSaida.Items.Add(ano);
      cbRegistroAnoEntrada.Items.Add(ano);

      ano := inttostr(ano.ToInteger + 1);
    end;
  end;

  cbRegistroAnoSaida.Text := formatdatetime('yyyy', now);
  cbRegistroAnoEntrada.Text := formatdatetime('yyyy', now);
end;
  ...
  try
    ...
    with query.Query2 do
      begin
        ...
        if cbDataEntradaRegistro.Text = 'Mês' then begin
          data_entrada := 'and month(data_entrada) = ' + dados.localizar_mes(cbDataEntradaMesRegistro.Text).ToString;
          ano_entrada := 'and year(data_entrada) = ' + cbRegistroAnoEntrada.Text;
        end else if cbDataEntradaRegistro.Text = 'Ano' then begin
          data_entrada := '';
          ano_entrada := 'and year(data_entrada) = ' + cbRegistroAnoEntrada.Text;
        end
        ...
        SQL.Add('obs_entrada as Observação');
...
begin
  try
    ...
    if cbDataSaidaRegistro.Text = 'Mês' then begin
      data_saida := 'and month(data_saida) = ' + dados.localizar_mes(cbDataSaidaMesRegistro.Text).ToString;
      ano_saida := 'and year(data_saida) = ' + cbRegistroAnoSaida.Text;
    end else if cbDataSaidaRegistro.Text = 'Ano' then begin
      data_saida := '';
      ano_saida := 'and year(data_saida) = ' + cbRegistroAnoSaida.Text;
    end
    ...
    with query.Query1 do
      begin
        ...
        SQL.Add('obs_saida as Observação');
        SQL.Add('from');
        SQL.Add('saidas');
...
procedure TFmlRegistros.dbGridSaidaRegistroDblClick(Sender: TObject);
begin
  doubleClickDbGrid;
end;
...
procedure TFmlRegistros.p_Click(Sender: TObject);
begin
  self.BringToFront;
end;
...
begin
  ...
  if cbDataEntradaRegistro.Text = 'Mês' then
    ...
    begin
      ...
      cbRegistroAnoEntrada.Visible :=  false;
    end;
