...
type
  Tfml_relatorio = class(TForm)
    ...
    Label28: TLabel;
    Label29: TLabel;
    cbAnoRelatorioSaidaEntrada: TComboBox;
    Label5: TLabel;
    Label7: TLabel;
...
begin
  ...
  if (ano.ToInteger <> ano_inicio.ToInteger) and (ano_inicio.ToInteger > 2015) then
    begin
      ...
      for I := 0 to High(sequencia_ano) do begin
        cbAnoRelatorioSaidaEntrada.Items.Add(inttostr(sequencia_ano[i]))
      end;
    end
    else
    begin
      for I := 0 to (ano.ToInteger - 2023) do begin
        cbAnoRelatorioSaidaEntrada.Items.Add(ano);
  ...
  cbAnoRelatorioSaidaEntrada.Text := formatdatetime('yyyy', now);
      ...
      begin
        for I := 0 to High(arrConta) do
          begin
            ...
            query.Query1.SQL.Add('select sum(VALOR_ENTRADA) as valorTotal from ENTRADAS where ID_CONTA = :pConta and month(DATA_ENTRADA) = :pMes and year(DATA_ENTRADA) = :pAno and ID_USUARIO = :pUser');
            ...
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            ...
            query.Query1.SQL.Clear;
            query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorGasto from SAIDAS where ID_CONTA = :pConta and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
            query.Query1.ParamByName('pConta').AsInteger := dados.localizar_id(arrConta[i], 4);
            query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
            ...
            query.Query1.SQL.Clear;
            query.Query1.SQL.Add('select sum(VALOR_ENTRADA) as valorTotalAnual from ENTRADAS where ID_CONTA = :pConta and year(DATA_ENTRADA) = :pAno  and ID_USUARIO = :pUser');
            query.Query1.ParamByName('pConta').AsInteger := dados.localizar_id(arrConta[i], 4);
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
            query.Query1.Open();
            ...
            query.Query1.SQL.Clear;
            query.Query1.SQL.Add('select distinct month(DATA_SAIDA) as meses from SAIDAS where year(DATA_SAIDA) = :pAno group by DATA_SAIDA');
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            query.Query1.Open();
            ...
            query.Query1.SQL.Clear;
            query.Query1.SQL.Add('select VALOR_ENTRADA as ultimaEntrada from entradas where ID_ENTRADA = (select max(ID_ENTRADA) from ENTRADAS where ID_CONTA = :pConta and month(DATA_ENTRADA) = :pMes and year(DATA_ENTRADA) = :pAno and ID_USUARIO = :pUser)');
            query.Query1.ParamByName('pConta').AsInteger := dados.localizar_id(arrConta[i], 4);
            query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
...
begin
  try
    ...
    for I := 0 to high(arrFinalidade) do
      begin
        query.Query1.SQL.Clear;
        query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal, avg(VALOR_SAIDA) as valorMedio from SAIDAS where ID_FINALIDADE = :pFinalidade and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
...
begin
  try
    query.Query1.SQL.Clear;
    query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal from SAIDAS where FIXO = ''s'' and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
    query.Query1.ParamByName('pMEs').AsInteger := dados.localizar_mes(cb_mes_relatorio.text);
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
    ...
    query.Query1.SQL.Clear;
    query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal from SAIDAS where FIXO = ''n'' and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
    query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
...
begin
  try
    query.Query1.SQL.Clear;
    query.Query1.SQL.Add('select sum(VALOR_ENTRADA) as valorTotal from ENTRADAS inner join ORIGEM on ENTRADAS.ID_ORIGEM = ORIGEM.ID_ORIGEM where ORIGEM.ORIGEM like ''investimento'' and month(DATA_ENTRADA) = :pMes and year(DATA_ENTRADA) = :pAno and ENTRADAS.ID_USUARIO = :pUser');
    query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
...
begin
  try
    query.Query1.SQL.Clear;
    query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal from SAIDAS inner join FINALIDADE on SAIDAS.ID_FINALIDADE = FINALIDADE.ID_FINALIDADE where FINALIDADE.FINALIDADE like ''investimentos'' and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and SAIDAS.ID_USUARIO = :pUser');
    query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
...
begin
  ...
  try
    ...
    query.Query1.SQL.Clear;
    query.Query1.SQL.Add('select SAIDAS.VALOR_SAIDA as valorTotal, ITEM.ITEM as item from SAIDAS inner join ITEM on SAIDAS.ID_ITEM = ITEM.ID_ITEM where FIXO = ''s'' and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and SAIDAS.ID_USUARIO = :pUser');
    query.Query1.ParamByName('pMEs').AsInteger := dados.localizar_mes(cb_mes_relatorio.text);
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
...
begin
  try
    ...
    for I := 0 to High(arrItem) do
      begin
        query.Query1.SQL.Clear;
        query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal, avg(VALOR_SAIDA) as valorMedio from SAIDAS where ID_ITEM = :pItem and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
    ...
    if cb_mes_relatorio.Text <> '' then
      begin
        for I := 0 to High(arrOrigem) do
          begin
            query.Query1.SQL.clear;
            query.Query1.SQL.Add('select sum(round(VALOR_ENTRADA, 2)) as valorTotal, avg(round(VALOR_ENTRADA, 2)) as mediaTotal from ENTRADAS where MONTH(DATA_ENTRADA) = :pMes and year(DATA_ENTRADA) = :pAno and ID_ORIGEM = :pOrigem and ID_USUARIO = :pUser');
            query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            ...
            query.Query1.SQL.Clear;
            query.Query1.SQL.Add('select max(VALOR_ENTRADA) as maiorValor from ENTRADAS where ID_ORIGEM = :pOrigem and year(DATA_ENTRADA) = :pAno and ID_USUARIO = :pUser');
            query.Query1.ParamByName('pOrigem').AsInteger := dados.localizar_id(arrOrigem[i], 3);
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
            query.Query1.open;
            ...
            query.Query1.SQL.Clear;
            query.Query1.SQL.Add('select VALOR_ENTRADA as ultimaEntrada from entradas where ID_ENTRADA = (select max(ID_ENTRADA) from ENTRADAS where ID_ORIGEM = :pOrigem and month(DATA_ENTRADA) = :pMes and year(DATA_ENTRADA) = :pAno and ID_USUARIO = :pUser)');
            query.Query1.ParamByName('pOrigem').AsInteger := dados.localizar_id(arrOrigem[i], 3);
            query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
            ...
            query.Query1.SQL.Clear;
            query.Query1.SQL.Add('select sum(VALOR_ENTRADA) as totalAnual from ENTRADAS where ID_ORIGEM = :pOrigem and year(DATA_ENTRADA) = :pAno and ID_USUARIO = :pUser');
            query.Query1.ParamByName('pOrigem').AsInteger := dados.localizar_id(arrOrigem[i], 3);
            query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
            query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
            query.Query1.Open();
...
begin
  ...
  try
    query.Query1.SQL.Clear;
    query.Query1.SQL.Add('select day(DATA_SAIDA) as data, sum(VALOR_SAIDA) as valorTotal from SAIDAS where FIXO = ''n'' and month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser group by DATA_SAIDA order by DATA_SAIDA desc');
    query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
...
begin
  try
    ...
    query.Query1.SQL.Clear;
    query.Query1.SQL.Add('select sum(VALOR_ENTRADA) as valorTotal from ENTRADAS inner join ORIGEM on ENTRADAS.ID_ORIGEM = ORIGEM.ID_ORIGEM  where ORIGEM.ORIGEM <> ''investimento'' and month(DATA_ENTRADA) = :pMes and year(DATA_ENTRADA) = :pAno and ENTRADAS.ID_USUARIO = :pUser');
    query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
    query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
    query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
    query.Query1.Open();
...
begin
  query.Query1.SQL.Clear;
  query.Query1.SQL.Add('select sum(VALOR_SAIDA) as valorTotal from SAIDAS where month(DATA_SAIDA) = :pMes and year(DATA_SAIDA) = :pAno and ID_USUARIO = :pUser');
  query.Query1.ParamByName('pMes').AsInteger := dados.localizar_mes(cb_mes_relatorio.Text);
  query.Query1.ParamByName('pAno').AsInteger := strtoint(cbAnoRelatorioSaidaEntrada.Text);
  query.Query1.ParamByName('pUser').AsInteger := id_usuarioLogado;
  query.Query1.Open();
